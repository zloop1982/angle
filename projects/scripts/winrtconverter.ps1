...
Function WinrtConverter($inFile, $outFile, $platform)
{
    ...
    if($winrt_globals.Node.ProjectName)
    {
        ...
        {
            $winrt_globals.Node.ProjectName = ""
        }
    }
    ...
    foreach($item in (Select-XML -Xml $xml_win32 -XPath '//ms:ItemGroup' -Namespace $ns))
    {
        ...
        elseif ($item.node.ProjectReference)
        {
            #$newNode = $xml_template.ImportNode($item.Node, $true)
            #$project.Node.AppendChild($newNode)
...
<#
    <ClCompile Include="opencv_core_pch.cpp">
      <PrecompiledHeader Condition="'$(Configuration)|$(Platform)'=='Debug|Win32'">Create</PrecompiledHeader>
      <PrecompiledHeaderFile Condition="'$(Configuration)|$(Platform)'=='Debug|Win32'">C:/Users/dalestam/Documents/GitHub/opencv-pr/modules/core/src/precomp.hpp</PrecompiledHeaderFile>
      <PrecompiledHeader Condition="'$(Configuration)|$(Platform)'=='Release|Win32'">Create</PrecompiledHeader>
      <PrecompiledHeaderFile Condition="'$(Configuration)|$(Platform)'=='Release|Win32'">C:/Users/dalestam/Documents/GitHub/opencv-pr/modules/core/src/precomp.hpp</PrecompiledHeaderFile>
      <PrecompiledHeader Condition="'$(Configuration)|$(Platform)'=='Debug|Arm'">Create</PrecompiledHeader>
      <PrecompiledHeaderFile Condition="'$(Configuration)|$(Platform)'=='Debug|Arm'">C:/Users/dalestam/Documents/GitHub/opencv-pr/modules/core/src/precomp.hpp</PrecompiledHeaderFile>
      <PrecompiledHeader Condition="'$(Configuration)|$(Platform)'=='Release|Arm'">Create</PrecompiledHeader>
      <PrecompiledHeaderFile Condition="'$(Configuration)|$(Platform)'=='Release|Arm'">C:/Users/dalestam/Documents/GitHub/opencv-pr/modules/core/src/precomp.hpp</PrecompiledHeaderFile>
    </ClCompile>
#>
    ...
    foreach($item in (Select-XML -Xml $xml_template -XPath '//ms:ItemDefinitionGroup' -Namespace $ns))
    {
        ...
        {
            AddElement $xml_template $item.Node.ClCompile 'PrecompiledHeaderFile'  $properties.Node.ClCompile.PrecompiledHeaderFile $ms
        }
        ...
        {
            AddElement $xml_template $item.Node.ClCompile 'ForcedIncludeFiles'  $properties.Node.ClCompile.ForcedIncludeFiles $ms
         }
    ...
    foreach($item in (Select-XML -Xml $xml_template -XPath '//ms:ItemGroup/ms:ClCompile/ms:PrecompiledHeader' -Namespace $ns))
    {
        $newNode = $xml_template.ImportNode($item.Node, $true)
    ...
    foreach($item in (Select-XML -Xml $xml_template -XPath '//ms:ItemGroup/ms:ClCompile/ms:PrecompiledHeaderFile' -Namespace $ns))
    {
        #$item.node.innerxml = '"' + (RelativePath $OutDir $item.node.innerxml) + '"'
        $newNode = $xml_template.ImportNode($item.Node, $true)
        $newNode.Condition = $newNode.Condition.replace("Win32", "ARM")
        $item.Node.parentNode.AppendChild($newNode)
        $newNode = $xml_template.ImportNode($item.Node, $true)
